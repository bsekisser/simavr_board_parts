	...
	avr_irq_t * cs_irq)
{
	...
	avr_irq_t* src = avr_io_getirq(part->avr, AVR_IOCTL_SPI_GETIRQ(0), SPI_IRQ_OUTPUT);
	avr_irq_t* dst = part->irq + IRQ_MICROCHIP_23K640_SPI_BYTE_IN;
	avr_connect_irq(src, dst);

	src = part->irq + IRQ_MICROCHIP_23K640_SPI_BYTE_OUT;
	dst = avr_io_getirq(part->avr, AVR_IOCTL_SPI_GETIRQ(0), SPI_IRQ_INPUT);
	avr_connect_irq(src, dst);

	dst = part->irq + IRQ_MICROCHIP_23K640_CS;
	avr_connect_irq(cs_irq, dst);
}

void
microchip_23k640_init(
		struct avr_t * avr,
		microchip_23k640_pp ppart)
{
	if(!(*ppart))
		*ppart = (microchip_23k640_p)malloc(sizeof(microchip_23k640_t));
	
	microchip_23k640_p part = *ppart;
